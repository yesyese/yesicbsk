...
async function main() {
  const wardenPassword = await bcrypt.hash("warden777", 10);
  const watchmanPassword = await bcrypt.hash("watchman777", 10);
  const AdminPassword = await bcrypt.hash("chairman777", 10);
